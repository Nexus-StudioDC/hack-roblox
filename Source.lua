...
-- POLAX GUI LOADER | TR & EN | MOBILE + PC
...
logo.Image = "rbxassetid://1049060234"


local title = Instance.new("TextLabel", top)
title.Size = UDim2.new(1,0,1,0)
title.BackgroundTransparency = 1
title.Text = "POLAX PANEL | SIFRELI GIRIS"
...
info.Text = "Sifreyi gir / Enter password"
...
btn.MouseButton1Click:Connect(function()
	if string.lower(box.Text) == "polax" then
		gui:Destroy()
		local f = loadstring(_G.POLAX_DEV_SCRIPT)
		if f then
			f()
		end
	else
		btn.Text = "YANLIS SIFRE / WRONG"
		task.wait(1)
		btn.Text = "GIRIS / LOGIN"
	end
end)
...
-- ================= SETTINGS SYSTEM (BLUR / THEME / ANIM / MOBILE) =================
-- Default theme: BLACK
-- Icons included via ImageLabels
...
local function applyTheme(root)
    for _,v in ipairs(root:GetDescendants()) do
        ...
        if v:IsA("TextLabel") or v:IsA("TextButton") then
            if SETTINGS.Theme == "BLACK" then
                v.TextColor3 = Color3.fromRGB(240,240,240)
            else
                v.TextColor3 = Color3.fromRGB(20,20,20)
            end
        end
    end
end
...
-- PAGE TRANSITION
local function switchPage(old,new)
    if old then
        TweenService:Create(old,TweenInfo.new(0.25),{Position=old.Position+UDim2.fromOffset(-40,0),Transparency=1}):Play()
        task.wait(0.2)
        old.Visible=false
    end
    if new then
        new.Position=new.Position+UDim2.fromOffset(40,0)
        new.Transparency=1
        new.Visible=true
        TweenService:Create(new,TweenInfo.new(0.25),{Position=new.Position-UDim2.fromOffset(40,0),Transparency=0}):Play()
    end
end
...
if UIS.TouchEnabled then
    for _,g in ipairs(game.CoreGui:GetChildren()) do
        if tostring(g.Name):lower():find("polax") then
            local s = Instance.new("UIScale", g)
            s.Scale = 1.1
        end
    end
end
...
-- ICON
local icon = Instance.new("ImageLabel", settingsPage)
icon.Size = UDim2.fromOffset(40,40)
icon.Position = UDim2.fromOffset(10,10)
icon.BackgroundTransparency = 1
icon.Image = "rbxassetid://7733964711" -- gear icon
...
makeToggle("BLUR TOGGLE",60,function()
    SETTINGS.Blur = not SETTINGS.Blur
    blur.Enabled = SETTINGS.Blur
end)

makeToggle("ANIM TOGGLE",110,function()
    SETTINGS.Animations = not SETTINGS.Animations
end)

makeToggle("THEME SWITCH",160,function()
    SETTINGS.Theme = (SETTINGS.Theme=="BLACK") and "WHITE" or "BLACK"
    for _,g in ipairs(game.CoreGui:GetChildren()) do
        if tostring(g.Name):lower():find("polax") then
            applyTheme(g)
        end
